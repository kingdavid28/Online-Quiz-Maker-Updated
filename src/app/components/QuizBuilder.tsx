  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error.message?.includes('schema') || 
          ...
          error.message?.includes('RPC fix') ||
          error.message?.includes('Reload schema') ||
          error.message?.includes('Database setup incomplete')) {
        setSchemaError(true);
        toast.error('Schema cache needs to be reloaded', {
          description: 'Click the modal for simple fix instructions',
          duration: 8000,
        });
      } else {
