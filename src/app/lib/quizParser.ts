...
export function parseQuizContent(content: string): ParseResult {
  ...
  try {
    ...
    // Try to parse as simple question list (one question per line)
    const simpleQuestions = parseSimpleFormat(lines);
    if (simpleQuestions.length > 0) {
      questions.push(...simpleQuestions);
      return { questions, errors, title };
    }

    errors.push('Could not detect a valid quiz format. Please use one of the supported formats shown in the examples.');
  } catch (error) {
...
/**
 * Parse simple question list (one question per line)
 * Example:
 * What is the capital of France?
 * What is 2+2?
 */
function parseSimpleFormat(lines: string[]): ParsedQuestion[] {
  const questions: ParsedQuestion[] = [];

  for (const line of lines) {
    // Skip very short lines or lines that don't look like questions
    if (line.length < 10) continue;
    
    // Check if the line looks like a question
    const looksLikeQuestion = 
      line.endsWith('?') ||
      line.match(/^(What|Who|When|Where|Why|How|Is|Are|Do|Does|Can|Will|Should|Could|Would|Name|Describe|Explain|Define|List)/i);
